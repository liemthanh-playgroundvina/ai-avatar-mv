...
class FaceAnimatePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        ref_image_tensor = self.ref_image_processor.preprocess(ref_image_tensor, height=height,
                                                               width=width)  # (bs, c, width, height)
        ref_image_tensor = ref_image_tensor.to(dtype=self.vae.dtype, device=self.vae.device)
        ref_image_latents = self.vae.encode(ref_image_tensor).latent_dist.mean
        ref_image_latents = ref_image_latents * 0.18215  # (b, 4, h, w)

        face_mask = face_mask.unsqueeze(1).to(dtype=self.face_locator.dtype,
                                              device=self.face_locator.device)  # (bs, f, c, H, W)
        ...
        face_mask = torch.cat([torch.zeros_like(face_mask), face_mask],
                              dim=0) if do_classifier_free_guidance else face_mask
